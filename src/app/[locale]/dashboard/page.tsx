...
function OwnerPanel({ buildings, locale }: { buildings: Array<{ id: string; name: string; address: string; area: string; joinCode: string; canManageUnitRequests: boolean; units: Array<{ id: string; label: string; type: string }> }>; locale: string }) {
  return <div className="grid items-stretch gap-6 lg:grid-cols-2">
    ...
    <Card className="h-full"><h2 className="text-xl font-bold">طلب ملكية شقة أو محل</h2><p className="mt-2 text-sm leading-6 text-[#52635b]">إذا لم تكن صاحب بناية، لا تحتاج إلى إنشاء بناية. أدخل رمز البناية ورقم الوحدة الذي أرسلهما لك صاحب البناية، وسيصل له طلب للموافقة.</p><form action={requestUnitOwnership} className="mt-4 grid gap-3"><input type="hidden" name="locale" value={locale} /><Input name="joinCode" label="رمز البناية" /><Input name="unitLabel" label="رقم الوحدة" /><Button>إرسال طلب ملكية</Button></form></Card>
  </div>;
}

function OwnerBuildingsPanel({ buildings }: { buildings: Array<{ id: string; name: string; address: string; area: string; joinCode: string; canManageUnitRequests: boolean; units: Array<{ id: string; label: string; type: string }> }> }) {
