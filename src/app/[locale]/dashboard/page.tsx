...
function OwnerPanel({ buildings, memberships, locale }: { buildings: Array<{ id: string; name: string; address: string; area: string; joinCode: string; units: Array<{ id: string; label: string; type: string }> }>; memberships: Array<{ id: string; tenant: { name: string; email: string }; unit: { label: string; building: { name: string } } }>; locale: string }) {
  return <div className="grid gap-6 lg:grid-cols-2">
    ...
    <Card className="lg:col-span-2"><h2 className="text-xl font-bold">مبانيك</h2>{buildings.length === 0 ? <p className="mt-2 text-[#52635b]">لم تضف مباني بعد.</p> : <ul className="mt-3 grid gap-3 md:grid-cols-2">{buildings.map((building) => <li key={building.id} className="rounded-2xl bg-[#f6f8f7] p-4"><strong>{building.name}</strong><p className="text-sm text-[#52635b]">{building.address} · رمز الانضمام: <code>{building.joinCode}</code></p><p className="mt-2 text-sm">الوحدات: {building.units.map((unit) => unit.label).join("، ") || "لا توجد"}</p></li>)}</ul>}</Card>
